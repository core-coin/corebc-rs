...
//         AggregatorInterface,
//         r#"[
//                 event AnswerUpdated(int256 indexed current, uint256 indexed roundId, uint256
// updatedAt)             ]"#,
//     );
...
//         .await
//         .unwrap();
//     let value: String = contract.method::<_, String>("getValue",
// ()).unwrap().call().await.unwrap();     assert_eq!(value, "hi");
// }
...
// client.clone()); // let simple_factory = ContractFactory::new(abi.clone(), bytecode.clone(),
// client2.clone()); // let not_so_simple_factory =
...
// // let simple_contract =
// //     simple_factory.deploy("the first
// one".to_string()).unwrap().legacy().send().await.unwrap(); // let not_so_simple_contract =
// not_so_simple_factory //     .deploy("the second one".to_string())
// //     .unwrap()
// //     .legacy()
// //     .send()
// //     .await
...
// //     .unwrap();
// // let get_value_call =
// //     reverting_contract.connect(client2.clone()).method::<_, String>("getValue",
// false).unwrap(); // let get_value_reverting_call =
