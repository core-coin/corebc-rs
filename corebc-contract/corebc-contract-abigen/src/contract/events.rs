...
impl Context {
    ...
    fn expand_events_enum(&self) -> TokenStream {
        ...
        quote! {
            ...
            impl #corebc_contract::EthLogDecode for #enum_name {
    ...
    fn expand_events_method(&self) -> Option<TokenStream> {
        ...
        if let Some(event) = iter.next() {
            ...
            Some(quote! {
                ...
                pub fn events(&self) -> #corebc_contract::builders::Event<
                    ::std::sync::Arc<M>,
                    M,
                    #ty,
                > {
    ...
    fn expand_filter(&self, event: &Event) -> TokenStream {
        ...
        quote! {
            #[doc = #doc_str]
            pub fn #function_name(&self) -> #corebc_contract::builders::Event<
                ::std::sync::Arc<M>,
                M,
                #struct_name
            > {
    ...
    fn expand_event(&self, event: &Event) -> Result<TokenStream> {
        ...
        Ok(quote! {
            #[derive(Clone, #corebc_contract::EthEvent, #corebc_contract::EthDisplay, #derives)]
    ...
    fn expand_transfer_filter_with_alias() {
        ...
        assert_quote!(cx.expand_filter(&event), {
            ...
            pub fn transfer_event_filter(
                &self
            ) -> ::corebc_contract::builders::Event<::std::sync::Arc<M>, M, TransferEventFilter>
            {
                self.0.event()
            }
        });
    ...
    fn expand_transfer_filter() {
        ...
        assert_quote!(cx.expand_filter(&event), {
            ...
            pub fn transfer_filter(
                &self
            ) -> ::corebc_contract::builders::Event<::std::sync::Arc<M>, M, TransferFilter>
            {
                self.0.event()
            }
        });
