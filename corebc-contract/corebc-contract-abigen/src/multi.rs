...
impl MultiAbigen {
    ...
    ///
    /// ```
    /// use corebc_contract_abigen::{MultiAbigen, SelectContracts};
    /// # fn t() {
    ...
    ///
    /// ```
    /// use corebc_contract_abigen::{ExcludeContracts, MultiAbigen};
    /// # fn t() {
    ///    let gen = MultiAbigen::from_json_files("./abi").unwrap().with_filter(
...
impl MultiBindingsInner {
    ...
    fn try_find_crate_version(&self) -> Result<String> {
        ...
        let ethers = toml
            .get("dependencies")
            .and_then(|v| v.get("ethers").or_else(|| v.get("corebc-contract")))
            .ok_or_else(|| eyre::eyre!("couldn't find ethers or corebc-contract dependency"))?;
