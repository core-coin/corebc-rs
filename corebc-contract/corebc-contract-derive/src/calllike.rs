...
pub fn derive_decode_impl(datatypes_array: TokenStream, trait_ident: Ident) -> TokenStream {
    ...
    quote! {
        ...
        if bytes.len() < 4 || bytes[..4] != <Self as #corebc_contract::#trait_ident>::selector() {
            return Err(#corebc_contract::AbiError::WrongSelector);
        }
        #data_types_init
    ...
    let codec_impl = quote! {
        ...
        impl #corebc_core::abi::AbiEncode for #struct_name {
            fn encode(self) -> ::std::vec::Vec<u8> {
                ...
                let selector = <Self as #corebc_contract::#trait_ident>::selector();
