...
async fn main() -> Result<()> {
    // Start a fresh local Shuttle node to execute the transfer on.
    let shuttle = Shuttle::new().spawn();
    // Use the first wallet autogenerated by the Shuttle node to send the transaction.
    let from_wallet: LocalWallet = shuttle.keys()[0].clone().into();
    let to_address = shuttle.addresses()[2].clone();

    // Deploy an ERC20 token contract on the freshly started local Shuttle node that doesn't have any
    ...
    let token_address = deploy_token_contract(&shuttle, from_wallet.clone()).await?;
    ...
    let signer = Arc::new(SignerMiddleware::new(
        provider,
        from_wallet.with_network_id(shuttle.network_id()),
    ));
...
/// Helper function to deploy a contract on a local shuttle node. See
...
async fn deploy_token_contract(
    ...
    let provider =
        Provider::<Http>::try_from(shuttle.endpoint())?.interval(Duration::from_millis(10u64));
    let client =
        SignerMiddleware::new(provider, deployer_wallet.with_network_id(shuttle.network_id()));
