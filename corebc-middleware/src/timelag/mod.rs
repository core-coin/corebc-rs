...
use corebc_providers::{Middleware, MiddlewareError};
    ...
    M: Middleware,
{
    ...
    async fn get_block_number(&self) -> Result<corebc_core::types::U64, Self::Error> {
        self.inner()
            .get_block_number()
            .await
            .map(|num| num - self.lag)
            .map_err(corebc_providers::MiddlewareError::from_err)
    }
    ...
    ) -> Result<corebc_providers::PendingTransaction<'_, Self::Provider>, Self::Error> {
        self.inner()
    ...
    async fn call(
        ...
        self.inner().call(tx, block).await.map_err(corebc_providers::MiddlewareError::from_err)
    }
    ...
    async fn get_code<T: Into<NameOrAddress> + Send + Sync>(
        ...
        self.inner().get_code(at, block).await.map_err(corebc_providers::MiddlewareError::from_err)
    }
    ...
    async fn get_logs(
        &self,
        filter: &corebc_core::types::Filter,
    ) -> Result<Vec<corebc_core::types::Log>, Self::Error> {
        let mut filter = filter.clone();
        filter.block_option = self.normalize_filter_range(filter.block_option).await?;

        self.inner().get_logs(&filter).await.map_err(corebc_providers::MiddlewareError::from_err)
    }

    async fn new_filter(
        &self,
        _filter: corebc_providers::FilterKind<'_>,
    ) -> Result<U256, Self::Error> {
    ...
    async fn watch_blocks(
        &self,
    ) -> Result<corebc_providers::FilterWatcher<'_, Self::Provider, TxHash>, Self::Error> {
        Err(TimeLagError::Unsupported)
    }
    ...
    async fn subscribe<T, R>(
        &self,
        _params: T,
    ) -> Result<corebc_providers::SubscriptionStream<'_, Self::Provider, R>, Self::Error>
    where
        T: std::fmt::Debug + serde::Serialize + Send + Sync,
        R: serde::de::DeserializeOwned + Send + Sync,
        Self::Provider: corebc_providers::PubsubClient,
    {
        Err(TimeLagError::Unsupported)
    }
    ...
    async fn subscribe_blocks(
        &self,
    ) -> Result<corebc_providers::SubscriptionStream<'_, Self::Provider, Block<TxHash>>, Self::Error>
    where
        Self::Provider: corebc_providers::PubsubClient,
    {
        Err(TimeLagError::Unsupported)
    ...
    async fn subscribe_pending_txs(
        &self,
    ) -> Result<corebc_providers::SubscriptionStream<'_, Self::Provider, TxHash>, Self::Error>
    where
        Self::Provider: corebc_providers::PubsubClient,
    {
        Err(TimeLagError::Unsupported)
    ...
    async fn subscribe_logs<'a>(
        &'a self,
        _filter: &corebc_core::types::Filter,
    ) -> Result<
        corebc_providers::SubscriptionStream<'a, Self::Provider, corebc_core::types::Log>,
        Self::Error,
    >
    where
        Self::Provider: corebc_providers::PubsubClient,
