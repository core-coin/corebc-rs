...
// use corebc_contract::{builders::ContractCall, BaseContract, ContractError};
// use corebc_core::{
//     abi::parse_abi,
...
//     "function execute(address target, bytes memory data) public payable returns (bytes memory
// response)"; /// The function signature of DsProxy's execute function, to deploy bytecode and
// execute data on it. const DS_PROXY_EXECUTE_CODE: &str =
//     "function execute(bytes memory code, bytes memory data) public payable returns (address
// target, bytes memory response)";
...
// ///
// /// ```no_run
// /// use corebc_middleware::{SignerMiddleware, transformer::DsProxy};
// /// use corebc_signers::LocalWallet;
// /// use corebc_providers::{Provider, Http};
// /// use corebc_core::types::{Address, Bytes};
...
// impl DsProxy {
//     /// Create a new instance of DsProxy by providing the address of the DsProxy contract that
...
//                     Some(addr) => *addr,
//                     None => panic!(
//                         "Must either be a supported Network ID or provide DsProxyFactory contract
// address"                     ),
//                 }
//             }
//         };
...
// contract,             // the others are logged by the deployed DsProxy contract and hence can be
// ignored.             let log = tx_receipt
//                 .logs
//                 .iter()
...
//     /// contract's address, or bytecode of a compiled smart contract. Depending on the target,
// the     /// appropriate `execute` method is called, that is, either
