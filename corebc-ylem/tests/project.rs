...
use corebc_ylem::{
    ...
    ProjectCompileOutput, ProjectPathsConfig, TestFileFilter, Ylem,
};
...
"#,
    ...
    // provide the library settings to let ylem link
    tmp.project_mut().ylem_config.settings.libraries = libs.with_applied_remappings(tmp.paths());
...
"#,
    ...
    let libs =
        Libraries::parse(&[format!("remapping/MyLib.sol:MyLib:{:?}", Address::zero())]).unwrap(); // provide the library settings to let ylem link
    tmp.project_mut().ylem_config.settings.libraries = libs.with_applied_remappings(tmp.paths());
...
fn can_detect_invalid_version() {
   ...
   "#;
    ...
    match out {
        YlemError::Message(err) => {
            assert_eq!(err, "Encountered invalid ylem version in src/A.sol: No ylem version exists that matches the version requirement: ^0.100.10");
        }
        _ => {
            unreachable!()
        }
...
fn can_sanitize_bytecode_hash() {
    let mut tmp = TempProject::dapptools().unwrap();
    tmp.project_mut().ylem_config.settings.metadata = Some(BytecodeHash::Ipfs.into());
...
fn can_compile_model_checker_sample() {
    ...
    project.project_mut().ylem_config.settings.model_checker = Some(ModelCheckerSettings {
...
#[tokio::test(flavor = "multi_thread")]
async fn can_install_ylem_and_compile_std_json_input_async() {
    let tmp = TempProject::dapptools_init().unwrap();
    tmp.assert_no_errors();
    let source = tmp.list_source_files().into_iter().find(|p| p.ends_with("Dapp.t.sol")).unwrap();
    let input = tmp.project().standard_json_input(source).unwrap();
    let ylem = &tmp.project().ylem;

    assert!(input.settings.remappings.contains(&"ds-test/=lib/ds-test/src/".parse().unwrap()));
    let input: CompilerInput = input.into();
    assert!(input.sources.contains_key(Path::new("lib/ds-test/src/test.sol")));

    remove_ylem_if_exists(&ylem.version().expect("failed to get version"));
...
fn can_detect_config_changes() {
   ...
   "#,
    ...
    project.project_mut().ylem_config.settings.optimizer.enabled = Some(true);
