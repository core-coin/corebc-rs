use crate::{
    ...
    error::{Result, YlemError, YlemIoError},
...
impl ProjectPathsConfig {
    ...
    pub fn current_hardhat() -> Result<Self> {
        Self::hardhat(std::env::current_dir().map_err(|err| YlemError::io(err, "."))?)
    }

    /// Creates a new config with the current directory as the root
    pub fn current_dapptools() -> Result<Self> {
        Self::dapptools(std::env::current_dir().map_err(|err| YlemError::io(err, "."))?)
    }
    ...
    pub fn create_all(&self) -> std::result::Result<(), YlemIoError> {
        if let Some(parent) = self.cache.parent() {
            fs::create_dir_all(parent).map_err(|err| YlemIoError::new(err, parent))?;
        }
        fs::create_dir_all(&self.artifacts)
            .map_err(|err| YlemIoError::new(err, &self.artifacts))?;
        fs::create_dir_all(&self.sources).map_err(|err| YlemIoError::new(err, &self.sources))?;
        fs::create_dir_all(&self.tests).map_err(|err| YlemIoError::new(err, &self.tests))?;
        fs::create_dir_all(&self.scripts).map_err(|err| YlemIoError::new(err, &self.scripts))?;
        for lib in &self.libraries {
            fs::create_dir_all(lib).map_err(|err| YlemIoError::new(err, lib))?;
        }
        Ok(())
    }
    ...
    /// ```
    ///  use std::path::Path;
    /// use corebc_ylem::ProjectPathsConfig;
    ...
    /// be provided to ylem via `--include-path` because it uses absolute imports.
    pub fn resolve_import_and_include_paths(
        ...
        let component = import
            .components()
            .next()
            .ok_or_else(|| YlemError::msg(format!("Empty import path {}", import.display())))?;
        ...
        if component == Component::CurDir || component == Component::ParentDir {
            ...
            utils::canonicalize(cwd.join(import)).map_err(|err| {
                YlemError::msg(format!("failed to resolve relative import \"{err:?}\""))
            })
        } else {
    ...
    fn flatten_node(
        ...
        let target_dir = target.parent().ok_or_else(|| {
            YlemError::msg(format!("failed to get parent directory for \"{:?}\"", target.display()))
        })?;
        ...
        let result = String::from_utf8(content).map_err(|err| {
            YlemError::msg(format!("failed to convert extended bytes to string: {err}"))
        })?;
...
impl ProjectPathsConfigBuilder {
    ...
    pub fn build(self) -> std::result::Result<ProjectPathsConfig, YlemIoError> {
        let root = self
            .root
            .clone()
            .map(Ok)
            .unwrap_or_else(std::env::current_dir)
            .map_err(|err| YlemIoError::new(err, "."))?;
...
impl YlemConfig {
    /// # Example
    ///
    /// Autodetect ylem version and default settings
    ///
    /// ```rust
    /// use corebc_ylem::YlemConfig;
    /// let config = YlemConfig::builder().build();
    /// ```
    pub fn builder() -> YlemConfigBuilder {
        YlemConfigBuilder::default()
    }
}

impl From<YlemConfig> for Settings {
    fn from(config: YlemConfig) -> Self {
        config.settings
    }
}

#[derive(Default)]
pub struct YlemConfigBuilder {
    settings: Option<Settings>,

    /// additionally selected outputs that should be included in the `Contract` that `ylem´ creates
    output_selection: Vec<ContractOutputSelection>,
}

impl YlemConfigBuilder {
    ...
    /// Creates the ylem config
    ///
    /// If no ylem version is configured then it will be determined by calling `ylem --version`.
    pub fn build(self) -> YlemConfig {
...
/// Container for all `--include-path` arguments for Ylem, see also
/// [Ylem docs](https://docs.soliditylang.org/en/v0.8.9/using-the-compiler.html#base-path-and-import-remapping).
///
...
/// Helper struct for serializing `--allow-paths` arguments to Ylem
///
/// From the [Ylem docs](https://docs.soliditylang.org/en/v0.8.9/using-the-compiler.html#base-path-and-import-remapping):
