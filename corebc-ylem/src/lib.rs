...
mod config;
pub use config::{AllowedLibPaths, PathStyle, ProjectPaths, ProjectPathsConfig, YlemConfig};
...
/// Represents a project workspace and handles `ylem` compiling of all contracts in that workspace.
#[derive(Debug)]
pub struct Project<T: ArtifactOutput = ConfigurableArtifacts> {
    ...
    /// Where to find ylem
    pub ylem: Ylem,
    /// How ylem invocation should be configured.
    pub ylem_config: YlemConfig,
    /// Whether caching is enabled
    pub cached: bool,
    /// Whether to output build information with each ylem call.
    pub build_info: bool,
    ...
    /// The paths which will be used with ylem's `--include-path` attribute
    pub include_paths: IncludePaths,
    /// Maximum number of `ylem` processes to run simultaneously.
    ylem_jobs: usize,
    /// Offline mode, if set, network access (download ylem) is disallowed
    pub offline: bool,
    /// Windows only config value to ensure the all paths use `/` instead of `\\`, same as `ylem`
    ///
...
impl Project {
    ...
    ///
    /// ```rust
    /// use corebc_ylem::{ConfigurableArtifacts, ProjectBuilder};
...
impl<T: ArtifactOutput> Project<T> {
    ...
    /// Applies the configured arguments to the given `Ylem`
    ///
    /// See [Self::configure_ylem_with_version()]
    pub(crate) fn configure_ylem(&self, ylem: Ylem) -> Ylem {
        let version = ylem.version().ok();
        self.configure_ylem_with_version(ylem, version, Default::default())
    }
    ...
    pub(crate) fn configure_ylem_with_version(
        &self,
        mut ylem: Ylem,
        version: Option<Version>,
        mut include_paths: IncludePaths,
    ) -> Ylem {
        if !ylem.args.iter().any(|arg| arg == "--allow-paths") {
            if let Some([allow, libs]) = self.allowed_paths.args() {
                ylem = ylem.arg(allow).arg(libs);
            }
        }
        if let Some(version) = version {
            if SUPPORTS_BASE_PATH.matches(&version) {
                ...
                if !base_path.is_empty() {
                    ylem = ylem.with_base_path(self.root());
                    if SUPPORTS_INCLUDE_PATH.matches(&version) {
                        ...
                        ylem = ylem.args(include_paths.args());
                    }
                }
            } else {
                ylem.base_path.take();
    ...
    /// Sets the maximum number of parallel `ylem` processes to run simultaneously.
    ///
    /// # Panics
    ///
    /// if `jobs == 0`
    pub fn set_ylem_jobs(&mut self, jobs: usize) {
        assert!(jobs > 0);
        self.ylem_jobs = jobs;
    }
    ...
    /// // configure the project with all its paths, ylem, cache etc. where the root dir is the current rust project.
    ...
    ///
    /// ```
    /// use corebc_ylem::{artifacts::Source, Project, utils};
    /// # fn demo(project: Project) {
    /// let project = Project::builder().build().unwrap();
        ...
        P: Into<PathBuf>,
    {
        ...
        let ylem = self.configure_ylem(self.ylem.clone());
        self.compile_with_version(&ylem, sources)
    }
    ...
    ///
    /// ```
    /// use corebc_ylem::{Project, TestFileFilter};
    /// # fn demo(project: Project) {
    /// let project = Project::builder().build().unwrap();
    /// let output = project
    ///     .compile_sparse(
    ...
    pub fn compile_sparse<F: FileFilter + 'static>(
        ...
        project::ProjectCompiler::with_sources_and_ylem(self, sources, self.ylem.clone())?
            .with_sparse_output(filter)
            .compile()
    }

    /// Compiles the given source files with the exact `Ylem` executable
    ///
    ...
    pub fn compile_with_version(
        ...
        project::ProjectCompiler::with_sources_and_ylem(self, sources, ylem.clone())?.compile()
    }
    ...
    pub fn cleanup(&self) -> std::result::Result<(), YlemIoError> {
        ...
        if self.paths.artifacts.exists() {
            std::fs::remove_dir_all(self.artifacts_path())
                .map_err(|err| YlemIoError::new(err, self.artifacts_path().clone()))?;
    ...
    pub fn standard_json_input(
        ...
        let target_index = graph.files().get(target).ok_or_else(|| {
            YlemError::msg(format!("cannot resolve file at {:?}", target.display()))
        })?;
        ...
        let mut settings = self.ylem_config.settings.clone();
...
pub struct ProjectBuilder<T: ArtifactOutput = ConfigurableArtifacts> {
    ...
    no_artifacts: bool,
    /// Whether automatic ylem version detection is enabled
    auto_detect: bool,
    /// Use offline mode
    offline: bool,
    ...
    /// All allowed paths for ylem's `--allowed-paths`
    allowed_paths: AllowedLibPaths,
    /// Paths to use for ylem's `--include-path`
    include_paths: IncludePaths,
    ylem_jobs: Option<usize>,
}
...
impl<T: ArtifactOutput> ProjectBuilder<T> {
    ...
    #[must_use]
    pub fn ylem(mut self, ylem: impl Into<Ylem>) -> Self {
        self.ylem = Some(ylem.into());
        self
    }

    #[must_use]
    pub fn ylem_config(mut self, ylem_config: YlemConfig) -> Self {
        self.ylem_config = Some(ylem_config);
        self
    }
    ...
    ///
    /// Prevents network possible access to download/check ylem installs
    #[must_use]
    ...
    ///
    /// If set to `true` all `\\` separators are replaced with `/`, same as ylem
    #[must_use]
    ...
    /// Sets automatic ylem version detection
    #[must_use]
    ...
    /// Disables automatic ylem version detection
    #[must_use]
    ...
    #[must_use]
    pub fn ylem_jobs(mut self, jobs: usize) -> Self {
        assert!(jobs > 0);
        self.ylem_jobs = Some(jobs);
        self
    }

    /// Sets the number of parallel `ylem` processes to `1`, no parallelization
    #[must_use]
    pub fn single_ylem_jobs(self) -> Self {
        self.ylem_jobs(1)
    }
    ...
    /// Adds an allowed-path to the ylem executable
    #[must_use]
    ...
    /// Adds multiple allowed-path to the ylem executable
    #[must_use]
    ...
    /// Adds an `--include-path` to the ylem executable
    #[must_use]
    ...
    /// Adds multiple include-path to the ylem executable
    #[must_use]
    ...
    pub fn build(self) -> Result<Project<T>> {
        ...
        let ylem = ylem.unwrap_or_default();
        let ylem_config = ylem_config.unwrap_or_else(|| YlemConfig::builder().build());
        ...
        Ok(Project {
            ...
            allowed_paths,
            include_paths,
            ylem_jobs: ylem_jobs.unwrap_or_else(num_cpus::get),
            offline,
            slash_paths,
        })
    }
