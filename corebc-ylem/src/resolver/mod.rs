...
//! contracts
//!
//! ## Ylem version auto-detection
//!
//! Solving a constraint graph is an NP-hard problem. The algorithm for finding the "best" solution
//! makes several assumptions and tries to find a version of "Ylem" that is compatible with all
//! source files.
//!
//! The algorithm employed here is fairly simple, we simply do a DFS over all the source files and
//! find the set of Ylem versions that the file and all its imports are compatible with, and then we
//! try to find a single Ylem version that is compatible with all the files. This is effectively the
//! intersection of all version sets.
//!
//! We always try to activate the highest (installed) ylem version first. Uninstalled ylem is only
//! used if this version is the only compatible version for a single file or in the intersection of
//! all version sets.
//!
//! This leads to finding the optimal version, if there is one. If there is no single Ylem version
...
//! latest (installed) Ylem version and try to minimize the number of Ylem versions used.
//!
//! ## Performance
//!
//! Note that this is a relatively performance-critical portion of the corebc-ylem preprocessing.
...
use crate::{
    error::Result, utils, IncludePaths, ProjectPathsConfig, Source, Sources, YlemError, YlemVersion,
};
...
///
/// This is kept separate from the `Graph` as the `Node`s get consumed when the `Ylem` to `Sources`
/// set is determined.
#[derive(Debug)]
...
impl GraphEdges {
    ...
    pub fn include_paths(&self) -> &IncludePaths {
        &self.resolved_ylem_include_paths
    }
...
impl Graph {
    ...
    pub fn resolve_sources(paths: &ProjectPathsConfig, sources: Sources) -> Result<Graph> {
        ...
        let mut resolved_ylem_include_paths = IncludePaths::default();
        ...
        while let Some((path, node)) = unresolved.pop_front() {
            ...
            for import in node.data.imports.iter() {
                ...
                match paths.resolve_import_and_include_paths(
                    cwd,
                    import_path,
                    &mut resolved_ylem_include_paths,
                ) {
                    Ok(import) => {
                        add_node(&mut unresolved, &mut index, &mut resolved_imports, import)
                            .map_err(|err| {
                                match err {
                                    err @ YlemError::ResolveCaseSensitiveFileName { .. } |
                                    err @ YlemError::Resolve(_) => {
                                        // make the error more helpful by providing additional
                                        // context
                                        YlemError::FailedResolveImport(
                                            Box::new(err),
                                            node.path.clone(),
                                            import_path.clone(),
                                        )
...
impl Graph {
    ...
    pub fn into_sources_by_version(self, offline: bool) -> Result<(VersionedSources, GraphEdges)> {
        ...
        /// However, it's not required to include them in the ylem `CompilerInput` as they would get
        /// picked up by ylem otherwise, but we add them, so we can create a corresponding
        ...
        // determine the `Sources` set for each ylem version
        ...
        Ok((
            VersionedSources {
                ...
                offline,
                resolved_ylem_include_paths: edges.resolved_ylem_include_paths.clone(),
            },
            edges,
        ))
    }
    ...
    fn retain_compatible_versions(&self, idx: usize, candidates: &mut Vec<&crate::YlemVersion>) {
    ...
    fn get_input_node_versions(
        &self,
        offline: bool,
    ) -> Result<HashMap<crate::YlemVersion, Vec<usize>>> {
        use crate::Ylem;
        ...
        let all_versions = if offline { Ylem::installed_versions() } else { Ylem::all_versions() };
        ...
        for idx in 0..self.edges.num_input_files {
            ...
            if candidates.is_empty() && !erroneous_nodes.contains(&idx) {
                    ...
                    self.node(idx).check_available_version(&all_versions, offline)
                {
                    ...
                    errors.push(format!("Encountered invalid ylem version in {f}: {version_err}"));
                } else {
        ...
        if versioned_nodes.len() == 1 {
            tracing::trace!(
                "found exact ylem version for all sources  \"{}\"",
        ...
        if errors.is_empty() {
            ...
            Err(YlemError::msg(errors.join("\n")))
        }
    }

    /// Tries to find the "best" set of versions to nodes, See [Ylem version
    /// auto-detection](#ylem-version-auto-detection)
    ///
    ...
    fn resolve_multiple_versions(
        all_candidates: Vec<(usize, HashSet<&crate::YlemVersion>)>,
    ) -> HashMap<crate::YlemVersion, Vec<usize>> {
        // returns the intersection as sorted set of nodes
        fn intersection<'a>(
            mut sets: Vec<&HashSet<&'a crate::YlemVersion>>,
        ) -> Vec<&'a crate::YlemVersion> {
            if sets.is_empty() {
                return Vec::new()
            }
        ...
        /// uninstalled version
        fn remove_candidate(candidates: &mut Vec<&crate::YlemVersion>) -> crate::YlemVersion {
        ...
        if !intersection.is_empty() {
            ...
            tracing::trace!(
                "resolved ylem version compatible with all sources  \"{}\"",
                exact_version
            );
        ...
        let mut versioned_nodes: HashMap<crate::YlemVersion, Vec<usize>> = HashMap::new();

        // try to minimize the set of versions, this is guaranteed to lead to `versioned_nodes.len()
        // > 1` as no ylem version exists that can satisfy all sources
        ...
        tracing::trace!(
            "no ylem version can satisfy all source files, resolved multiple versions  \"{:?}\"",
            versioned_nodes.keys()
        );
...
pub struct VersionedSources {
    resolved_ylem_include_paths: IncludePaths,
    inner: HashMap<crate::YlemVersion, Sources>,
    offline: bool,
}
...
impl VersionedSources {
    ...
    ///
    /// This will also configure following ylem arguments:
    ///    - `allowed_paths`
    ///    - `base_path`
    pub fn get<T: crate::ArtifactOutput>(
        self,
        project: &crate::Project<T>,
    ) -> Result<std::collections::BTreeMap<crate::Ylem, (semver::Version, Sources)>> {
        use crate::Ylem;
        // we take the installer lock here to ensure installation checking is done in sync
        #[cfg(any(test, feature = "tests"))]
        let _lock = crate::compile::take_ylem_installer_lock();

        let mut sources_by_version = std::collections::BTreeMap::new();
        for (version, sources) in self.inner {
            let ylem = if !version.is_installed() {
                if self.offline {
                    return Err(YlemError::msg(format!(
                        "missing ylem \"{version}\" installation in offline mode"
                    )))
                } else {
                    // install missing ylem
            ...
            if self.offline {
                tracing::trace!(
                    "skip verifying ylem checksum for {} in offline mode",
                    ylem.ylem.display()
                );
            } else {
                tracing::trace!("verifying ylem checksum for {}", ylem.ylem.display());
                if ylem.verify_checksum().is_err() {
                    tracing::trace!("corrupted ylem version, redownloading  \"{}\"", version);
                    Ylem::blocking_install(version.as_ref())?;
                    tracing::trace!("reinstalled ylem: \"{}\"", version);
                }
            }

            let version = ylem.version()?;

            // this will configure the `Ylem` executable and its arguments
            let ylem = project.configure_ylem_with_version(
                ylem,
                Some(version.clone()),
                self.resolved_ylem_include_paths.clone(),
            );
            sources_by_version.insert(ylem, (version, sources));
        }
    ...
    pub fn read(file: impl AsRef<Path>) -> Result<Self> {
        ...
        let source = Source::read(file).map_err(|err| {
            ...
            if !exists && err.path().is_symlink() {
                YlemError::ResolveBadSymlink(err)
            } else {
                ...
                if !exists {
                    ...
                    if let Some(existing_file) = find_case_sensitive_existing_file(file) {
                        YlemError::ResolveCaseSensitiveFileName { error: err, existing_file }
                    } else {
                        YlemError::Resolve(err)
                    }
                } else {
                    YlemError::Resolve(err)
                }
...
/// Errors thrown when checking the ylem version of a file
#[derive(Debug, thiserror::Error)]
#[allow(unused)]
enum SourceVersionError {
    #[error("Failed to parse solidity version {0}: {1}")]
    InvalidVersion(String, semver::Error),
    #[error("No ylem version exists that matches the version requirement: {0}")]
    NoMatchingVersion(VersionReq),
    #[error("No ylem version installed that matches the version requirement: {0}")]
