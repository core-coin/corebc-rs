...
use crate::{error::YlemError, YlemIoError};
...
pub fn canonicalize(path: impl AsRef<Path>) -> Result<PathBuf, YlemIoError> {
    ...
    res.map_err(|err| YlemIoError::new(err, path))
}
...
///
/// # fn main() {
/// use corebc_ylem::utils::common_ancestor_all;
...
///
/// # fn main() {
/// use corebc_ylem::utils::common_ancestor;
/// let foo = Path::new("/foo/bar/foo");
/// let bar = Path::new("/foo/bar/bar");
...
pub(crate) fn tempdir(name: &str) -> Result<tempfile::TempDir, YlemIoError> {
    tempfile::Builder::new().prefix(name).tempdir().map_err(|err| YlemIoError::new(err, name))
}

/// Reads the json file and deserialize it into the provided type
pub fn read_json_file<T: DeserializeOwned>(path: impl AsRef<Path>) -> Result<T, YlemError> {
    let path = path.as_ref();
    let file = std::fs::File::open(path).map_err(|err| YlemError::io(err, path))?;
...
pub fn create_parent_dir_all(file: impl AsRef<Path>) -> Result<(), YlemError> {
...
contract A {}
        ...
        matches!(found, YlemError::ResolveCaseSensitiveFileName { .. });
    }
