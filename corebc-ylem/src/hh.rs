...
pub struct HardhatArtifact {
    ...
    /// The bytecode's link references object as returned by ylem. If the contract doesn't need to
    /// be linked, this value contains an empty object.
    #[serde(default)]
    pub link_references: BTreeMap<String, BTreeMap<String, Vec<Offsets>>>,
    /// The deployed bytecode's link references object as returned by ylem. If the contract doesn't
