...
//! based on its `ExtraOutputValues` that maps to various objects in the ylem contract output, see
...
use crate::{
    ...
    Artifact, ArtifactOutput, SourceFile, YlemConfig, YlemError,
};
...
pub struct ConfigurableArtifacts {
    ...
    /// ```rust
    /// 
    /// use corebc_ylem::{ExtraOutputFiles, ConfigurableArtifacts};
...
impl ConfigurableArtifacts {
    ...
    pub fn settings(&self) -> Settings {
        YlemConfig::builder().additional_outputs(self.output_selection()).build().into()
    }
...
impl ArtifactOutput for ConfigurableArtifacts {
    ...
    fn write_contract_extras(&self, contract: &Contract, file: &Path) -> Result<(), YlemError> {
...
pub struct ExtraOutputValues {
    ...
    /// ```rust
    /// 
    /// use corebc_ylem::ExtraOutputValues;
...
pub struct ExtraOutputFiles {
    ...
    /// ```rust
    /// 
    /// use corebc_ylem::ExtraOutputFiles;
    ...
    pub fn write_extras(&self, contract: &Contract, file: &Path) -> Result<(), YlemError> {
        ...
        if self.ir_optimized {
            if let Some(ref iropt) = contract.ir_optimized {
                ...
                fs::write(&file, iropt).map_err(|err| YlemError::io(err, file))?
            }
        }
        ...
        if self.ir {
            if let Some(ref ir) = contract.ir {
                ...
                fs::write(&file, ir).map_err(|err| YlemError::io(err, file))?
            }
        }
        ...
        if self.assembly {
            if let Some(ref evm) = contract.evm {
                if let Some(ref asm) = evm.assembly {
                    ...
                    fs::write(&file, asm).map_err(|err| YlemError::io(err, file))?
                }
            }
        }
            ...
            if let Some(ref evm) = contract.evm {
                if let Some(ref bytecode) = evm.bytecode {
                    if let Some(ref sourcemap) = bytecode.source_map {
                        ...
                        fs::write(&file, sourcemap).map_err(|err| YlemError::io(err, file))?
                    }
                }
            }
        }
        ...
        if self.bytecode {
            if let Some(ref code) = contract.get_bytecode_bytes() {
                ...
                fs::write(&file, code).map_err(|err| YlemError::io(err, file))?
            }
        }
