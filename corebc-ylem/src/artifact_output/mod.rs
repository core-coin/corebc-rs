...
use crate::{
    ...
    utils, HardhatArtifact, ProjectPathsConfig, SolFilesCache, YlemError, YlemIoError,
};
use corebc_core::{abi::Abi, types::Bytes};
use semver::Version;
...
pub struct ArtifactId {
    ...
    pub source: PathBuf,
    /// `ylem` version that produced this artifact
    pub version: Version,
}
...
impl<T: Serialize> ArtifactFile<T> {
    ...
    pub fn write(&self) -> Result<()> {
        ...
        fs::write(&self.file, serde_json::to_vec_pretty(&self.artifact)?)
            .map_err(|err| YlemError::io(err, &self.file))?;
        Ok(())
    }
}
...
impl<T> Artifacts<T> {
    ...
    ///
    /// *Note:* if there are multiple artifacts (contract compiled with different ylem) then this
...
pub trait Artifact {
        ...
        Self: Sized,
    {
        ...
        Ok((
            abi.ok_or_else(|| YlemError::msg("abi missing"))?,
            bytecode.ok_or_else(|| YlemError::msg("bytecode missing"))?,
            deployed_bytecode.ok_or_else(|| YlemError::msg("deployed bytecode missing"))?,
        ))
    }
...
/// Handler invoked with the output of `ylem`
///
...
/// The project compiler may invoke different `ylem` executables on the same
...
/// [`crate::VersionedContracts`] also tracks the `contract` to (`artifact` + `ylem version`)
/// relationship (1-N+).
pub trait ArtifactOutput {
    ...
    /// Handle the aggregated set of compiled contracts from the ylem [`crate::CompilerOutput`].
    ///
    /// This will be invoked with all aggregated contracts from (multiple) ylem `CompilerOutput`.
    ...
    fn on_output(
        ...
        fs::create_dir_all(&layout.artifacts).map_err(|err| {
            ...
            YlemIoError::new(err, &layout.artifacts)
        })?;
    ...
    fn read_cached_artifact(path: impl AsRef<Path>) -> Result<Self::Artifact> {
        ...
        let file = fs::File::open(path).map_err(|err| YlemError::io(err, path))?;
    ...
    ///
    /// We do this because not all `SourceFile`s emitted by ylem have at least 1 corresponding entry
    /// in the `contracts`
    /// section of the ylem output. For example for an `errors.sol` that only contains custom error
    /// definitions and no contract, no `Contract` object will be generated by ylem. However, we
...
impl ArtifactOutput for MinimalCombinedArtifactsHardhatFallback {
    ...
    fn read_cached_artifact(path: impl AsRef<Path>) -> Result<Self::Artifact> {
        let path = path.as_ref();
        let content = fs::read_to_string(path).map_err(|err| YlemError::io(err, path))?;
