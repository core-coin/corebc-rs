//! Ylem artifact types
use crate::{
    compile::*, error::YlemIoError, remappings::Remapping, utils, ProjectPathsConfig, YlemError,
};
...
/// A set of different Ylem installations with their version and the sources to be compiled
pub(crate) type VersionedSources = BTreeMap<Ylem, (Version, Sources)>;

/// A set of different Ylem installations with their version and the sources to be compiled
pub(crate) type VersionedFilteredSources = BTreeMap<Ylem, (Version, FilteredSources)>;

const SOLIDITY: &str = "Solidity";
const YUL: &str = "Yul";

/// Input type `ylem` expects
...
impl CompilerInput {
    ...
    pub fn new(path: impl AsRef<Path>) -> Result<Vec<Self>, YlemIoError> {
    ...
    ///
    /// A [CompilerInput] expects a language setting, supported by ylem are solidity or yul.
    ...
    /// sources _and_ all paths in ylem settings such as remappings
    ///
    /// See also `ylem --base-path`
...
impl Settings {
    ...
    ///
    /// ```
    /// use corebc_ylem::artifacts::output_selection::ContractOutputSelection;
    /// use corebc_ylem::artifacts::Settings;
...
impl Libraries {
    ...
    ///
    /// ```
    /// use corebc_ylem::artifacts::Libraries;
    /// let libs = Libraries::parse(&[
    ///     "src/DssSpell.sol:DssExecLib:0xfD88CeE74f7D78697775aBDAE53f9Da1559728E4".to_string(),
    /// ])
    /// .unwrap();
    /// ```
    pub fn parse(libs: &[String]) -> Result<Self, YlemError> {
        let mut libraries = BTreeMap::default();
        for lib in libs {
            let mut items = lib.split(':');
            let file = items.next().ok_or_else(|| {
                YlemError::msg(format!("failed to parse path to library file: {lib}"))
            })?;
            let lib = items
                .next()
                .ok_or_else(|| YlemError::msg(format!("failed to parse library name: {lib}")))?;
            let addr = items
                .next()
                .ok_or_else(|| YlemError::msg(format!("failed to parse library address: {lib}")))?;
    ...
    /// Ylem expects the lib paths to match the global path after remappings were applied
    ///
...
///
/// Ylem's default is `Ipfs`, see <https://docs.soliditylang.org/en/latest/using-the-compiler.html#compiler-api>.
...
/// Bindings for [`ylem` contract metadata](https://docs.soliditylang.org/en/latest/metadata.html)
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Metadata {
...
/// metadata value that's being hashed by ylem
...
/// Model checker settings for ylem
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
impl Source {
    ...
    pub fn read(file: impl AsRef<Path>) -> Result<Self, YlemIoError> {
        let file = file.as_ref();
        Ok(Self::new(fs::read_to_string(file).map_err(|err| YlemIoError::new(err, file))?))
    }

    /// Recursively finds all source files under the given dir path and reads them all
    pub fn read_all_from(dir: impl AsRef<Path>) -> Result<Sources, YlemIoError> {
    ...
    pub fn read_all_files(files: Vec<PathBuf>) -> Result<Sources, YlemIoError> {
        Self::read_all(files)
    }

    /// Reads all files
    pub fn read_all<T, I>(files: I) -> Result<Sources, YlemIoError>
    where
    ...
    pub fn par_read_all<T, I>(files: I) -> Result<Sources, YlemIoError>
    where
        I: IntoIterator<Item = T>,
...
impl Source {
    ...
    pub async fn async_read(file: impl AsRef<Path>) -> Result<Self, YlemIoError> {
        let file = file.as_ref();
        Ok(Self::new(
            tokio::fs::read_to_string(file).await.map_err(|err| YlemIoError::new(err, file))?,
        ))
    }

    /// Finds all source files under the given dir path and reads them all
    pub async fn async_read_all_from(dir: impl AsRef<Path>) -> Result<Sources, YlemIoError> {
        Self::async_read_all(utils::source_files(dir.as_ref())).await
    }

    /// async version of `Self::read_all`
    pub async fn async_read_all<T, I>(files: I) -> Result<Sources, YlemIoError>
    where
        I: IntoIterator<Item = T>,
        T: Into<PathBuf>,
    {
...
impl CompilerOutput {
    ...
    {
        // Note: use `to_lowercase` here because ylem not necessarily emits the exact file name,
...
/// A helper type that ensures lossless (de)serialisation unlike [`corebc_core::abi::Abi`] which
/// omits some information of (nested) components in a serde roundtrip. This is a problem for
/// abienconderv2 structs because [`corebc_core::abi::Contract`]'s representation of those are
/// [`corebc_core::abi::Param`] and the `kind` field of type [`corebc_core::abi::ParamType`] does
...
/// this type keeps a copy of the [`serde_json::Value`] when deserialized from the `ylem` json
...
pub enum UserDocNotice {
    // NOTE: this a variant used for constructors on older ylem versions
    Constructor(String),
...
impl SourceFiles {
    ...
    /// use corebc_ylem::artifacts::SourceFiles;
...
mod tests {
  ...
  "sources": { }
    ...
    fn can_link_bytecode() {
        ...
        let bytecode =  "6060604052341561000f57600080fd5b60f48061001d6000396000f300606060405260043610603e5763ffffffff7c010000000000000000000000000000000000000000000000000000000060003504166326121ff081146043575b600080fd5b3415604d57600080fd5b60536055565b005b73____lib2.sol:L______________________________";
        ...
        let hashed_placeholder = "6060604052341561000f57600080fd5b60f48061001d6000396000f300606060405260043610603e5763ffffffff7c010000000000000000000000000000000000000000000000000000000060003504166326121ff081146043575b600080fd5b3415604d57600080fd5b60536055565b005b73____$4f8f038d0f25cb3cca2df12f58365b834d$____";
