...
use corebc_core::{abi::Address, types::Bytes};
...
impl CompactBytecode {
    ...
    pub fn link(
        ...
        if let Some((key, mut contracts)) = self.link_references.remove_entry(file) {
            println!("111111111file: {:?}, library {:?}, address {:?}, contracts {:?}", file, library, address, contracts);
            if contracts.remove(library).is_some() {
                println!("'''''''''''''''''''''''''''''''");
                self.object.link(file, library, address);
            }
            println!("222222222222file: {:?}, library {:?}, address {:?}, contracts {:?}", file, library, address, contracts);
            if !contracts.is_empty() {
                println!("2222222'''''''''''''''''''''''''''''''");

                self.link_references.insert(key, contracts);
            }
            println!("33333333333file: {:?}, library {:?}, address {:?}, contracts {:?}", file, library, address, contracts);
            if self.link_references.is_empty() {
                println!("33333333333'''''''''''''''''''''''''''''''");
                return self.object.resolve().is_some()
            }
            println!("file: {:?}, library {:?}, address {:?}, contracts {:?}", file, library, address, contracts);
...
impl BytecodeObject {
    ...
    pub fn link_fully_qualified(&mut self, name: impl AsRef<str>, addr: Address) -> &mut Self {
        if let BytecodeObject::Unlinked(ref mut unlinked) = self {
            ...
            // the hash. This is also still supported by `ylem` so we handle this as well
...
/// adds by default.
///
/// This ensures that we serialize bytecode data in the same way as ylem does, See also <https://github.com/gakonst/ethers-rs/issues/1422>
