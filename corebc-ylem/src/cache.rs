...
use crate::{
    artifacts::Sources,
    config::{ProjectPaths, YlemConfig},
...
///
/// `corebc-ylem` uses a different format version id, but the actual format is consistent with
/// hardhat This allows corebc-ylem to detect if the cache file was written by hardhat or
/// `corebc-ylem`
...
impl SolFilesCache {
    ...
    /// ```
    /// # fn t() {
    /// use corebc_ylem::cache::SolFilesCache;
    ...
    pub fn write(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        let file = fs::File::create(path).map_err(|err| YlemError::io(err, path))?;
        tracing::trace!(
    ...
    /// ```
    /// # fn t() {
    /// use corebc_ylem::artifacts::contract::CompactContract;
    /// use corebc_ylem::cache::SolFilesCache;
    /// use corebc_ylem::Project;
    /// let project = Project::builder().build().unwrap();
    ...
    pub fn read_artifact<Artifact: DeserializeOwned>(
        ...
        let artifact_path =
            self.find_artifact_path(contract_file, contract_name).ok_or_else(|| {
                YlemError::ArtifactNotFound(contract_file.to_path_buf(), contract_name.to_string())
            })?;
...
impl SolFilesCache {
    pub async fn async_read(path: impl AsRef<Path>) -> Result<Self> {
        let path = path.as_ref();
        let content =
            tokio::fs::read_to_string(path).await.map_err(|err| YlemError::io(err, path))?;
    ...
    pub async fn async_write(&self, path: impl AsRef<Path>) -> Result<()> {
        ...
        tokio::fs::write(path, content).await.map_err(|err| YlemError::io(err, path))
    }
}
...
/// `ylem` versions generating version specific artifacts.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
...
pub struct CacheEntry {
    ...
    ///
    /// In theory a file can be compiled by different ylem versions:
...
impl CacheEntry {
    ...
    /// ```
    /// use corebc_ylem::cache::CacheEntry;
    ...
    pub fn read_last_modification_date(file: impl AsRef<Path>) -> Result<u64> {
        ...
        let last_modification_date = fs::metadata(file)
            .map_err(|err| YlemError::io(err, file.to_path_buf()))?
            .modified()
            .map_err(|err| YlemError::io(err, file.to_path_buf()))?
...
impl<'a, T: ArtifactOutput> ArtifactsCacheInner<'a, T> {
    ...
    fn create_cache_entry(&self, file: &Path, source: &Source) -> CacheEntry {
        ...
        let entry = CacheEntry {
            ...
            ylem_config: self.project.ylem_config.clone(),
            imports,
    ...
    /// criteria - which ylem also requires in order to compile a dirty file.
    ///
    ...
    fn is_dirty(&self, file: &Path, version: &Version) -> bool {
        if let Some(hash) = self.content_hashes.get(file) {
            if let Some(entry) = self.cache.entry(file) {
                ...
                if self.project.ylem_config != entry.ylem_config {
                    tracing::trace!("changed ylem config for source file \"{}\"", file.display());
                    return true
                }
    ...
    pub fn consume(
        ...
        match self {
            ...
            ArtifactsCache::Cached(cache) => {
                ...
                // this step is necessary because the concrete artifacts are only known after ylem
