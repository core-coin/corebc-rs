...
pub enum SparseOutputFilter {
    ...
    ///
    /// In other words, we request the output of ylem only for files that have been detected as
    /// _dirty_.
    #[default]
    AllDirty,
...
impl SparseOutputFilter {
    /// While ylem needs all the files to compile the actual _dirty_ files, we can tell ylem to
