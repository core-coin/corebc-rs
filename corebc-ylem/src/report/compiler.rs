...
//! As ylem compiler input and output can become quite large (in the tens of MB) we still want a way
...
/// Debug Helper type that can be used to write the [crate::Ylem] [CompilerInput] and
/// [CompilerOutput] to disk if configured.
///
/// # Example
///
/// If `corebc_ylem_LOG=in=in.json,out=out.json` is then the reporter will be configured to write
/// the compiler input as pretty formatted json to `in.{ylem version}.json` and the compiler output
/// to `out.{ylem version}.json`
///
/// ```no_run
/// use corebc_ylem::report::YlemCompilerIoReporter;
/// std::env::set_var("corebc_ylem_LOG", "in=in.json,out=out.json");
/// let rep = YlemCompilerIoReporter::from_default_env();
/// ```
#[derive(Debug, Clone, Default)]
pub struct YlemCompilerIoReporter {
    /// where to write the output to, `None` if not enabled
    target: Option<Target>,
}

impl YlemCompilerIoReporter {
    /// Returns a new `YlemCompilerIOLayer` from the fields in the given string,
    /// ignoring any that are invalid.
    pub fn new(value: impl AsRef<str>) -> Self {
        Self { target: Some(value.as_ref().parse().unwrap_or_default()) }
    }

    /// `corebc_ylem_LOG` is the default environment variable used by
    /// [`YlemCompilerIOLayer::from_default_env`]
    ///
    /// [`YlemCompilerIOLayer::from_default_env`]: #method.from_default_env
    pub const DEFAULT_ENV: &'static str = "corebc_ylem_LOG";

    /// Returns a new `YlemCompilerIOLayer` from the value of the `corebc_ylem_LOG` environment
    /// variable, ignoring any invalid filter directives.
    pub fn from_default_env() -> Self {
        Self::from_env(Self::DEFAULT_ENV)
    }

    /// Returns a new `YlemCompilerIOLayer` from the value of the given environment
    /// variable, ignoring any invalid filter directives.
...
impl<S> From<S> for YlemCompilerIoReporter
where
    S: AsRef<str>,
{
    fn from(s: S) -> Self {
...
mod tests {
    ...
    fn can_init_reporter_from_env() {
        ...
        std::env::remove_var("corebc_ylem_LOG");
    }
}
