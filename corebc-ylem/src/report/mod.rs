...
use crate::{remappings::Remapping, CompilerInput, CompilerOutput, Ylem};
use semver::Version;
use std::{
    any::{Any, TypeId},
    cell::RefCell,
...
mod compiler;
pub use compiler::YlemCompilerIoReporter;
...
/// ```rust
/// use corebc_ylem::report::BasicStdoutReporter;
/// let subscriber = corebc_ylem::report::init(BasicStdoutReporter::default());
/// ```
...
pub trait Reporter: 'static + std::fmt::Debug {
    /// Callback invoked right before [`Ylem::compile()`] is called
    ///
    /// This contains the [Ylem] its [Version] the complete [CompilerInput] and all files that
    /// triggered the compile job. The dirty files are only provided to give a better feedback what
    /// was actually compiled.
    ///
    /// If caching is enabled and there has been a previous successful ylem run, the dirty files set
    ...
    /// Invoked with the `CompilerOutput` if [`Ylem::compile()`] was successful
    fn on_ylem_success(
        &self,
        _ylem: &Ylem,
        _version: &Version,
    ...
    fn on_ylem_installation_start(&self, _version: &Version) {}

    /// Invoked after a new [`Ylem`] bin was successfully installed
    fn on_ylem_installation_success(&self, _version: &Version) {}

    /// Invoked after a [`Ylem`] installation failed
    fn on_ylem_installation_error(&self, _version: &Version, _error: &str) {}
...
pub(crate) fn ylem_spawn(
    ylem: &Ylem,
    version: &Version,
    input: &CompilerInput,
    dirty_files: &[PathBuf],
) {
    get_default(|r| r.reporter.on_ylem_spawn(ylem, version, input, dirty_files));
}

pub(crate) fn ylem_success(
    ylem: &Ylem,
    version: &Version,
    output: &CompilerOutput,
    duration: &Duration,
) {
    get_default(|r| r.reporter.on_ylem_success(ylem, version, output, duration));
}

#[allow(unused)]
pub(crate) fn ylem_installation_start(version: &Version) {
    get_default(|r| r.reporter.on_ylem_installation_start(version));
}

#[allow(unused)]
pub(crate) fn ylem_installation_success(version: &Version) {
    get_default(|r| r.reporter.on_ylem_installation_success(version));
}

#[allow(unused)]
pub(crate) fn ylem_installation_error(version: &Version, error: &str) {
    get_default(|r| r.reporter.on_ylem_installation_error(version, error));
}
...
impl Default for BasicStdoutReporter {
    fn default() -> Self {
        Self { ylem_io_report: YlemCompilerIoReporter::from_default_env() }
    }
}
