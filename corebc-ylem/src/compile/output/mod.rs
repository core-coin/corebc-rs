...
use crate::{
    ...
    ArtifactId, ArtifactOutput, Artifacts, CompilerOutput, ConfigurableArtifacts, YlemIoError,
};
...
impl<T: ArtifactOutput> ProjectCompileOutput<T> {
    ...
    /// use corebc_ylem::ConfigurableContractArtifact;
    /// use corebc_ylem::{ArtifactId, Project};
    ///
    ...
    /// ```no_run
    /// use std::collections::btree_map::BTreeMap;
    /// use corebc_ylem::{ConfigurableContractArtifact, Project};
    ///
    /// let project = Project::builder().build().unwrap();
    ...
    /// [`crate::ConfigurableContractArtifact::source_file()`]), is only available via the ylem
    /// `CompilerOutput`
    ...
    /// Get the (merged) ylem compiler output
    /// ```no_run
    /// use std::collections::btree_map::BTreeMap;
    /// use corebc_ylem::artifacts::contract::Contract;
    /// use corebc_ylem::Project;
    ///
    /// let project = Project::builder().build().unwrap();
    ...
    /// Returns the set of `Artifacts` that were compiled with `ylem` in
    /// [`crate::Project::compile()`]
    ...
    /// ```
    /// use corebc_ylem::Project;
    /// use corebc_ylem::artifacts::*;
    /// use corebc_ylem::info::ContractInfo;
    ///
...
impl ProjectCompileOutput<ConfigurableArtifacts> {
    ...
    /// ```no_run
    /// use std::collections::btree_map::BTreeMap;
    /// use corebc_ylem::artifacts::contract::CompactContractBytecode;
    /// use corebc_ylem::{ArtifactId, Project};
    /// use corebc_ylem::contracts::ArtifactContracts;
    ///
    /// let project = Project::builder().build().unwrap();
...
///
/// This is effectively a ylem version aware `CompilerOutput`
#[derive(Clone, Debug, Default, PartialEq)]
pub struct AggregatedCompilerOutput {
    /// all errors from all `CompilerOutput`
    pub errors: Vec<Error>,
    /// All source files combined with the ylem version used to compile them
    pub sources: VersionedSourceFiles,
    /// All compiled contracts combined with the ylem version used to compile them
    pub contracts: VersionedContracts,
    // All the `BuildInfo`s of ylem invocations.
...
impl AggregatedCompilerOutput {
    ...
    ///
    /// The created files have the md5 hash `{_format,ylemVersion,ylemLongVersion,input}` as their
    /// file name
    pub fn write_build_infos(&self, build_info_dir: impl AsRef<Path>) -> Result<(), YlemIoError> {
        if self.build_infos.is_empty() {
            return Ok(())
        }
        let build_info_dir = build_info_dir.as_ref();
        std::fs::create_dir_all(build_info_dir)
            .map_err(|err| YlemIoError::new(err, build_info_dir))?;
        for (version, build_info) in &self.build_infos {
            trace!("writing build info file for ylem {}", version);
            let file_name = format!("{}.json", build_info.id);
            let file = build_info_dir.join(file_name);
            std::fs::write(&file, &build_info.build_info)
                .map_err(|err| YlemIoError::new(err, file))?;
        }
        Ok(())
    }
...
/// Helper type to implement display for ylem errors
#[derive(Clone, Debug)]
