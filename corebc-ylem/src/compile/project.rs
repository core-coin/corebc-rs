...
//!
//! The final step is invoking `Ylem` via the standard JSON format.
//!
...
//! ```
//!
//! If compiled with `ylem github.com/ethereum/dapp-bin/=dapp-bin/` the compiler will look for the
...
//! successful ylem build. The [cache file](crate::cache::SolFilesCache) stores metadata for all the
//! files that were provided to ylem.
...
//! artifacts emitted by a file. A solidity file can also be compiled with several ylem versions.
//!
//! For example in `A(<=0.8.10) imports C(>0.4.0)` and
//! `B(0.8.11) imports C(>0.4.0)`, both `A` and `B` import `C` but there's no ylem version that's
...
//! dirty, files need to be passed to ylem. A file will be considered as dirty if:
...
//! ylem with only these dirty files instead of the entire source set.
...
pub struct ProjectCompiler<'a, T: ArtifactOutput> {
    ...
    /// How to select ylem [`crate::artifacts::CompilerOutput`] for files
...
impl<'a, T: ArtifactOutput> ProjectCompiler<'a, T> {
    ...
    /// appropriate `Ylem` -> `Sources` set as well as the compile mode to use (parallel,
    /// sequential)
    ///
    ...
    pub fn with_sources(project: &'a Project<T>, sources: Sources) -> Result<Self> {
        ...
        let sources = if project.ylem_jobs > 1 && sources_by_version.len() > 1 {
            // if there are multiple different versions, and we can use multiple jobs we can compile
            // them in parallel
            CompilerSources::Parallel(sources_by_version, project.ylem_jobs)
        } else {
    ...
    /// Compiles the sources with a pinned `Ylem` instance
    pub fn with_sources_and_ylem(
        project: &'a Project<T>,
        sources: Sources,
        ylem: Ylem,
    ) -> Result<Self> {
        ...
        // make sure `ylem` has all required arguments
        ...
        let sources_by_version = BTreeMap::from([(ylem, (version, sources))]);
        let sources = CompilerSources::Sequential(sources_by_version);

        Ok(Self { edges, project, sources, sparse_output: Default::default() })
    }

    /// Applies the specified filter to be applied when selecting ylem output for
    ...
    fn preprocess(self) -> Result<PreprocessedState<'a, T>> {
        ...
        // convert paths on windows to ensure consistency with the `CompilerOutput` `ylem` emits,
        // which is unix style `/`
        sources.slash_paths();
...
/// Represents the state after `ylem` was successfully invoked
#[derive(Debug)]
...
impl<'a, T: ArtifactOutput> CompiledState<'a, T> {
    ...
    fn write_artifacts(self) -> Result<ArtifactsState<'a, T>> {
        ...
        {
            trace!("skip writing cache file due to ylem errors: {:?}", output.errors);
            project.artifacts_handler().output_to_artifacts(
                &output.contracts,
                &output.sources,
                ctx,
...
/// Determines how the `ylem <-> sources` pairs are executed
#[derive(Debug, Clone)]
#[allow(dead_code)]
...
impl CompilerSources {
    ...
    ///
    /// This effectively ensures that `ylem` can find imported files like `/src/Cheats.sol` in the
...
impl FilteredCompilerSources {
    /// Compiles all the files with `Ylem`
    fn compile(
        self,
        settings: &Settings,
...
/// Compiles the input set sequentially and returns an aggregated set of the ylem `CompilerOutput`s
fn compile_sequential(
    ...
    for (ylem, (version, filtered_sources)) in input {
        if filtered_sources.is_empty() {
            // nothing to compile
            trace!("skip ylem {} {} for empty sources set", ylem.as_ref().display(), version);
            continue
        }
        trace!(
            "compiling {} sources with ylem \"{}\" {:?}",
        ...
        for input in CompilerInput::with_sources(sources) {
            ...
            if actually_dirty.is_empty() {
                ...
                // language set
                trace!(
                    "skip ylem {} {} compilation of {} compiler input due to empty source set",
                    ylem.as_ref().display(),
                    version,
                    input.language
                );
            ...
            report::ylem_spawn(&ylem, &version, &input, &actually_dirty);
            let output = ylem.compile(&input)?;
            report::ylem_success(&ylem, &version, &output, &start.elapsed());
...
fn compile_parallel(
    ...
    trace!("compile {} sources in parallel using up to {} ylem jobs", input.len(), num_jobs);
    ...
    for (ylem, (version, filtered_sources)) in input {
        ...
        for input in CompilerInput::with_sources(sources) {
            ...
            jobs.push((ylem.clone(), version.clone(), job, actually_dirty))
        }
    }
    ...
    // start a rayon threadpool that will execute all `Ylem::compile()` processes
    let pool = rayon::ThreadPoolBuilder::new().num_threads(num_jobs).build().unwrap();

    let outputs = pool.install(move || {
        jobs.into_par_iter()
            .map(move |(ylem, version, input, actually_dirty)| {
                ...
                let start = Instant::now();
                report::ylem_spawn(&ylem, &version, &input, &actually_dirty);
                ylem.compile(&input).map(move |output| {
                    report::ylem_success(&ylem, &version, &output, &start.elapsed());
