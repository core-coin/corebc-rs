use crate::*;
use corebc_etherscan::account::{InternalTxQueryOption, TokenQueryOption};
    ...
    run_with_client(Network::Mainnet, |client| async move {
        let balance = client
            .get_ether_balance_single(
                &"0x000058eB28A67731c570Ef827C365c89B5751F9E6b0a".parse().unwrap(),
                None,
            )
            .await;
        balance.unwrap();
...
async fn get_transactions_success() {
    run_with_client(Network::Mainnet, |client| async move {
        let txs = client
            .get_transactions(
                &"0x40000F26FfBe5F04ED43630fdC30A87638d53D0b0876".parse().unwrap(),
                None,
            )
            .await;
        txs.unwrap();
...
// CORETODO: need to fix blockchain explorer
// #[tokio::test]
// #[serial]
...
//                     "0x00002c1ba59d6f58433fb1eaee7d20b26ed83bda51a3".parse().unwrap(),
//                 ),
//                 None,
//             )
//             .await;
...
//                     "0x00004e83362442b8d1bec281594cea3050c8eb01311c".parse().unwrap(),
//                 ),
//                 None,
//             )
//             .await
...
//                     "0x00006975be450864c02b4613023c2152ee0743572325".parse().unwrap(),
//                     "0x000006012c8cf97bead5deae237070f9587f8e7a266d".parse().unwrap(),
//                 ),
//                 None,
//             )
//             .await;
...
//                 TokenQueryOption::ByAddressAndContract(
//                     "0x0000216CD350a4044e7016f14936663e2880Dd2A39d7".parse().unwrap(),
//                     "0x0000495f947276749ce646f68ac8c248420045cb7b5e".parse().unwrap(),
//                 ),
//                 None,
//             )
//             .await;
...
//         client
//             .get_mined_blocks(
//                 &"0x00009dd134d14d1e65f84b706d6f205cd5b1cd03a46b".parse().unwrap(),
//                 None,
//                 None,
//             )
//             .await
...
async fn get_devin_transactions() {
    run_with_client(Network::Devin, |client| async move {
        let txs = client
            .get_transactions(
                &"0x00001549ea9b546ba9ffb306d78a1e1f304760cc4abf".parse().unwrap(),
                None,
            )
            .await;
        txs.unwrap();
