...
impl Client {
    ...
    ///
    /// ```no_run
    /// use corebc_etherscan::account::InternalTxQueryOption;
    ///
    /// # async fn foo(client: corebc_etherscan::Client) -> Result<(), Box<dyn std::error::Error>> {
    ...
    ///
    /// ```no_run
    /// use corebc_etherscan::account::TokenQueryOption;
    ///
    /// # async fn foo(client: corebc_etherscan::Client) -> Result<(), Box<dyn std::error::Error>> {
    /// let address = "0x00004e83362442b8d1bec281594cea3050c8eb01311c".parse()?;
