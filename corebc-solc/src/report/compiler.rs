...
/// # Example
///
/// If `corebc_solc_LOG=in=in.json,out=out.json` is then the reporter will be configured to write
/// the compiler input as pretty formatted json to `in.{solc version}.json` and the compiler output
/// to `out.{solc version}.json`
///
/// ```no_run
/// use corebc_solc::report::SolcCompilerIoReporter;
/// std::env::set_var("corebc_solc_LOG", "in=in.json,out=out.json");
...
impl SolcCompilerIoReporter {
    ...
    /// `corebc_solc_LOG` is the default environment variable used by
    /// [`SolcCompilerIOLayer::from_default_env`]
    ///
    /// [`SolcCompilerIOLayer::from_default_env`]: #method.from_default_env
    pub const DEFAULT_ENV: &'static str = "corebc_solc_LOG";

    /// Returns a new `SolcCompilerIOLayer` from the value of the `corebc_solc_LOG` environment
...
mod tests {
    ...
    fn can_init_reporter_from_env() {
        ...
        std::env::remove_var("corebc_solc_LOG");
    }
}
