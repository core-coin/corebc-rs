...
impl Project {
    ...
    ///
    /// ```rust
    /// use corebc_solc::{ConfigurableArtifacts, ProjectBuilder};
...
impl<T: ArtifactOutput> Project<T> {
    ...
    ///
    /// ```
    /// use corebc_solc::{artifacts::Source, Project, utils};
    /// # fn demo(project: Project) {
    /// let project = Project::builder().build().unwrap();
    ...
    ///
    /// ```
    /// use corebc_solc::{Project, TestFileFilter};
    /// # fn demo(project: Project) {
    /// let project = Project::builder().build().unwrap();
    /// let output = project
    ///     .compile_sparse(
