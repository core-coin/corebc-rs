...
///
/// `corebc-solc` uses a different format version id, but the actual format is consistent with
/// hardhat This allows corebc-solc to detect if the cache file was written by hardhat or
/// `corebc-solc`
...
impl SolFilesCache {
    ...
    /// ```
    /// # fn t() {
    /// use corebc_solc::cache::SolFilesCache;
    /// use corebc_solc::Project;
    ///
    ...
    /// ```
    /// # fn t() {
    /// use corebc_solc::artifacts::contract::CompactContract;
    /// use corebc_solc::cache::SolFilesCache;
    /// use corebc_solc::Project;
    /// let project = Project::builder().build().unwrap();
    ...
    /// use corebc_solc::cache::SolFilesCache;
    /// use corebc_solc::Project;
    /// use corebc_solc::artifacts::contract::CompactContractBytecode;
    /// # fn t() {
    /// let project = Project::builder().build().unwrap();
    /// let cache = SolFilesCache::read_joined(&project.paths).unwrap();
...
impl CacheEntry {
    ...
    /// ```
    /// use corebc_solc::cache::CacheEntry;
