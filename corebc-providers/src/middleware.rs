...
/// use corebc_core::types::{U64, TransactionRequest, U256, transaction::eip2718::TypedTransaction, BlockId};
/// use thiserror::Error;
/// use async_trait::async_trait;
///
/// #[derive(Debug)]
...
pub trait Middleware: Sync + Send + Debug {
    ...
    /// ```no_run
    /// # use corebc_providers::{Provider, Http, Middleware};
    /// use corebc_providers::erc::ERCNFT;
    /// # async fn foo(provider: Provider<Http>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// return an error.
    ///
    /// [`H256`]: corebc_core::types::H256
    /// [`Log`]: corebc_core::types::Log
    ///
    /// This method is hidden because filter lifecycle  should be managed by
    /// the [`FilterWatcher`]
    #[doc(hidden)]
